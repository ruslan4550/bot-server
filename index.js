...
const i18n = {
    ...
    tr: { /* Türkçe tam metin (az'dan fərqli olan yerlər aşağıda)*/ },
    en: { /* English */ },
    ru: { /* Русский */ }
};

// Dil fayllarını tam olaraq burada göstərmək çox yer tutar – az dilinə uyğun olaraq bütün açarları doldurun.
// (Siz artıq tam i18n obyektini yuxarıdakı kodda yerləşdirmisiniz.)
    ...
    if (settings) {
        if (settings.botDescription !== undefined && settings.botDescription !== currentDesc) {
            ...
            try {
                ...
                console.log("Bot təsviri yeniləndi.");
        ...
        if (settings.botShortDescription !== undefined && settings.botShortDescription !== currentShortDesc) {
            ...
            try {
                ...
                console.log("Qısa təsvir yeniləndi.");
            } catch(e) { console.error(e); }
        }
        // Bot profil şəkli
        if (settings.botProfilePhoto && settings.botProfilePhoto !== lastProfilePhoto) {
            ...
            try {
                await bot.setChatPhoto(chatId, settings.botProfilePhoto); // bot-un öz ID-sini tapmaq lazımdır
                // Bunun üçün bot.getMe() istifadə edib bot id-ni alırıq:
                const botInfo = await bot.getMe();
                await bot.setChatPhoto(botInfo.id, settings.botProfilePhoto);
            } catch(e) { console.error("Profil şəkli yenilənə bilmədi:", e.message); }
        }
    }
}, 15000);

let lastProfilePhoto = null;
...
// ------------------------------------------------------------------
// Mesaj yönləndirmə (media dəstəyi ilə)
// ------------------------------------------------------------------
async function forwardMessageToTargets(client, messageToForward, groups) {
    for (const g of groups) {
        try {
            const target = await resolveTargetEntity(client, g);
            await client.forwardMessages(target, {
                messages: [messageToForward.id],
                fromPeer: messageToForward.peerId,
            });
        } catch (err) {
            console.error(`(${messageToForward.peerId}) -> ${g} XƏTA:`, err.message);
        }
    }
}
...
bot.on('callback_query', async (query) => {
  ...
  if (data.startsWith("toggle_")) {
      ...
      if (acc) {
        ...
        // Yenidən manage_numbers callback-i çağır
  ...
  // Avtomatik cavab mesajını silmək üçün əlavə düymə
...
bot.on('message', async (msg) => {
  if (!msg.text || msg.text.startsWith('/')) {
    // Avtomatik cavab yoxlaması (mətn olmayan mesajlar üçün də işləsin)
    const chatId = msg.chat.id;
    const user = await getDB(`users/${chatId}`);
    const lang = user?.lang || "az";
    const state = await getDB(`users/${chatId}/state`);
    if (!state || state === "IDLE") {
        const autoReply = await getDB(`users/${chatId}/autoReplyMessage`);
        if (autoReply) {
  ...
  // Boş vaxtda avtomatik cavab
  const autoReply = await getDB(`users/${chatId}/autoReplyMessage`);
  if (autoReply && (!state || state === "IDLE")) {
    bot.sendMessage(chatId, autoReply);
  }
});
    ...
    for (const phoneKey in user.accounts) {
        ...
        try {
          ...
          if (sourceMessages && sourceMessages.length > 0) {
            ...
            await forwardMessageToTargets(client, msgToForward, groups);
